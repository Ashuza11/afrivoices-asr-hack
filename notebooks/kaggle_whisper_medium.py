# %% [markdown]
# # AfriVoices ASR — Whisper-medium Fine-tune (Kaggle P100)
# Experiment A: encoder-decoder, 307M params, same multilingual approach as whisper-small baseline.
...
# %% [code]
import os, io, json, tarfile, lzma, glob, time, pickle
...
HF_REPO_ID   = "Ash11/afrivoices-whisper-medium-all6"
CACHE_DIR    = "/kaggle/working/records"
CKPT_DIR     = "/kaggle/working/whisper-medium-all6"
...
# %% [code]
SWA_CACHE  = f"{CACHE_DIR}/swa_records.pkl"
SWA_TARGET = 8000
...
# %% [code]
SOM_CACHE  = f"{CACHE_DIR}/som_records.pkl"
SOM_TARGET = 4000
...
# %% [code]
ANV_CACHE    = f"{CACHE_DIR}/anv_records.pkl"
...
if len(_cached) >= ANV_TARGET * len(TARGET_LANGS):
    ...
    anv_records = [r for lang in TARGET_LANGS for r in buckets[lang]]
    print(f"ANV total: {len(anv_records)} clips.")
...
user    = api.whoami()
repo_id = f"{user['name']}/afrivoices-whisper-medium-all6"
...
# %% [code]
ft_model     = WhisperForConditionalGeneration.from_pretrained(CKPT_DIR).to(device)
ft_processor = WhisperProcessor.from_pretrained(CKPT_DIR)
ft_model.eval()
...
LANG_MAP = {"swa": "sw", "som": "so", "kik": None, "luo": None, "mas": None, "kln": None}
...
for pf_idx, pq_path in enumerate(all_parquet_files):
    ...
    print(f"[{pf_idx+1}/{len(all_parquet_files)}] {fname}  ({elapsed:.1f}m elapsed, ETA {eta:.0f}m)", flush=True)
...
sub[["id", "language", "transcription"]].to_csv("/kaggle/working/submission.csv", index=False)
